...
public:
    ...
    inline Tquaternion operator*(const Tquaternion &q) const {
        ...
        return Tquaternion(w1 * x2 + x1 * w2 + y1 * z2 - z1 * y2, w1 * y2 + y1 * w2 + z1 * x2 - x1 * z2, w1 * z2 + z1 * w2 + x1 * y2 - y1 * x2, w1 * w2 - x1 * x2 - y1 * y2 - z1 * z2);
    }
...
public:
    ...
    inline Tmat3() { }
    inline Tmat3(const my_type &that)
        : base(that) { }
    inline Tmat3(const base &that)
        : base(that) { }
...
static inline mat4 ortho(float left, float right, float bottom, float top, float n, float f) {
    return mat4(vec4(2.0f / (right - left), 0.0f, 0.0f, 0.0f), vec4(0.0f, 2.0f / (top - bottom), 0.0f, 0.0f), vec4(0.0f, 0.0f, 2.0f / (n - f), 0.0f), vec4((left + right) / (left - right), (bottom + top) / (bottom - top), (n + f) / (f - n), 1.0f));
}
